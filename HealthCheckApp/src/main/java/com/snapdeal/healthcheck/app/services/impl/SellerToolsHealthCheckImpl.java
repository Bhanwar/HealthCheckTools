...
public class SellerToolsHealthCheckImpl implements Callable<HealthCheckResult>{

	private final Logger log = LoggerFactory.getLogger(getClass());
	
	private String endPoint;
	
	public SellerToolsHealthCheckImpl(String endPoint) {
		this.endPoint = endPoint;
	}
	@Override
	public HealthCheckResult call() throws Exception {
		boolean isServerUp = false;
		String url = endPoint + "/healthcheck";
		HealthCheckResult result = new HealthCheckResult(Component.SELLERTOOLS.code());
		log.debug("Checking if MRG Seller Tools server is up on endpoint: " + endPoint);
