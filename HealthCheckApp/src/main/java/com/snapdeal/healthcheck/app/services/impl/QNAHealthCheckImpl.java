...
public class QNAHealthCheckImpl implements Callable<HealthCheckResult>{

	private final Logger log = LoggerFactory.getLogger(getClass());
	
	private String endPoint;
	
	public QNAHealthCheckImpl(String endPoint) {
		this.endPoint = endPoint;
	}
	@Override
	public HealthCheckResult call() throws Exception {
		boolean isServerUp = false;
		String url = endPoint + "/api/service/qna/admin/v1/report/deephealth";
		HealthCheckResult result = new HealthCheckResult(Component.QNA.code());
		log.debug("Checking if QnA server is up on endpoint: " + endPoint);
