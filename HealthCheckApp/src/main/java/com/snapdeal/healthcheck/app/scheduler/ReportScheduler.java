...
public class ReportScheduler extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		try {
			...
			String header = "<html><h2>Health Check Daily report for date: " + date + ", as on: " + time
					+ "</h2><h2>Environment: " + envName + "</h2>";
			...
			if (!dwnTymDataMap.isEmpty()) {
				...
				for (Entry<String, List<DownTimeData>> dwnTymEntrySet : dwnTymDataMap.entrySet()) {
					...
					for (DownTimeData data : dwnTymEntrySet.getValue()) {
						String downTimeMins = data.getTotalDownTimeInMins();
						if (downTimeMins == null) {
							long totalTimeMins = (currentDate.getTime() - data.getDownTime().getTime()) / 60000;
							downTimeMins = String.valueOf(totalTimeMins);
						}
						html.append("<tr style=\"text-align: center\">");
						if (addComp) {
							html.append(
									"<td rowspan=\"" + rowCount + "\" style=\"border: 1px solid black; padding: 5px;\">"
											+ getStringForHtml(dwnTymEntrySet.getKey()) + "</td>");
							addComp = false;
						}
						html.append("<td style=\"border: 1px solid black; padding: 5px;\">"
								+ getStringForHtml(data.getDownTime()) + "</td>");
						html.append("<td style=\"border: 1px solid black; padding: 5px;\">"
								+ getStringForHtml(data.getUpTime()) + "</td>");
						html.append("<td style=\"border: 1px solid black; padding: 5px;\">"
								+ getStringForHtml(downTimeMins) + "</td>");
						html.append("<td style=\"border: 1px solid black; padding: 5px;\">"
								+ getStringForHtml(data.getReasonCode()) + "</td>");
						html.append("<td style=\"border: 1px solid black; padding: 5px;\">"
								+ getStringForHtml(data.getDescription()) + "</td>");
						html.append("</tr>");
					}
				}
				html.append("</table>");
			...
			MailHtmlData.sendHtmlMail(toAddress, ccAddress, envName + " health check daily report - " + date,
					html.toString(), sendMail);
		} catch (Exception e) {
			log.error("Exception occured while running daily report scheduler!!", e);
		}
	}
