...
public class KAMHealthCheckImpl implements Callable<HealthCheckResult>{

	private final Logger log = LoggerFactory.getLogger(getClass());
	
	private String endPoint;
	
	public KAMHealthCheckImpl(String endPoint) {
	...
	public HealthCheckResult call() throws Exception {
		...
		String url = endPoint + "/api/service/v1/accountmanagers?acctMgrID=test1.kam@snapdeal.com";
		HealthCheckResult result = new HealthCheckResult(Component.KAM.code());
		log.debug("Checking if KAM server is up on endpoint: " + endPoint);
		HttpCallResponse resp = callGet(url);
		if (resp.getStatusCode() != null && resp.getStatusCode().equals("200 OK") && resp.getResponseBody() != null
				&& resp.getResponseBody().contains("reportingManager"))
			isServerUp = true;
