...
public class SHIPFARHealthCheckImpl implements Callable<HealthCheckResult>{

	private final Logger log = LoggerFactory.getLogger(getClass());
	
	private String endPoint;
	
	public SHIPFARHealthCheckImpl(String endPoint) {
	...
	public HealthCheckResult call() throws Exception {
		...
		HealthCheckResult result = new HealthCheckResult(Component.SHIPFAR.code());
		log.debug("Checking if Ship Far server is up on endpoint: " + endPoint);
		HttpCallResponse resp = callGet(url);
		if (resp.getStatusCode() != null && resp.getStatusCode().equals("200 OK") && resp.getResponseBody() != null) {
			isServerUp = JsonPath.read(resp.getResponseBody(), "$.app.description").toString().contains("Running on");
		}
